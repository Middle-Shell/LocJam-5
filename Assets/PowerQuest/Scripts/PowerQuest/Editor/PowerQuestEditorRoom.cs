	...
	void LayoutHotspotGUI(Rect rect, int index, bool isActive, bool isFocused)
	{
		if ( m_selectedRoom != null && m_selectedRoom.GetHotspotComponents().IsIndexValid(index))
		{
			...
			{		
				...
				{
					if ( GUI.Button(new Rect(offset, rect.y, fixedWidth, EditorGUIUtility.singleLineHeight), "Wiz", QuestEditorUtils.GetMiniButtonStyle(actionNum++,actionCount) ) )
					{
						// Lookat
				...
				{
					if ( GUI.Button(new Rect(offset, rect.y, fixedWidth, EditorGUIUtility.singleLineHeight), "Prin", QuestEditorUtils.GetMiniButtonStyle(actionNum++,actionCount) ) )
					{
						// Interact
						QuestScriptEditor.Open( m_selectedRoom, QuestScriptEditor.eType.Hotspot,
	...
	void LayoutRoomCharacterGUI(Rect rect, int index, bool isActive, bool isFocused)
	{
		if ( m_powerQuest != null && m_powerQuest.GetCharacterPrefabs().IsIndexValid(index))
		{
			...
			if ( itemComponent != null && itemComponent.GetData() != null )
			{
				...
				if ( PowerQuestEditor.GetActionEnabled(eQuestVerb.Look) )
				{
					if ( GUI.Button(new Rect(offset, rect.y, fixedWidth, EditorGUIUtility.singleLineHeight), "Wiz", QuestEditorUtils.GetMiniButtonStyle(actionNum++,actionCount+1) ) )
					{
						// Lookat
				...
				if ( PowerQuestEditor.GetActionEnabled(eQuestVerb.Use) )
				{
					if ( GUI.Button(new Rect(offset, rect.y, fixedWidth, EditorGUIUtility.singleLineHeight), "Prin", QuestEditorUtils.GetMiniButtonStyle(actionNum++,actionCount+1) ) )
					{
						// Interact
