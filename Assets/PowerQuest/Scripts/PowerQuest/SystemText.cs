...
{ 
	...
	public string ImportCSVWebResult = null;
	
	#if UNITY_WEBGL && !UNITY_EDITOR
	bool m_waitingForCSVWeb = false;
	#endif
	public IEnumerator ImportFromCSVWeb(string scriptPath)
	{
		//bool result = false;
		ImportCSVWebResult = null;

		if ( string.IsNullOrEmpty(scriptPath) )
		{
			ImportCSVWebResult = "Invalid Path";
			yield break;
		}
		...
		*/
			...
			m_waitingForCSVWeb = true;
		
			string csvText = null;

			UnityEngine.Networking.UnityWebRequest request = UnityEngine.Networking.UnityWebRequest.Get(scriptPath);
				request.SendWebRequest().completed += ((item)=>
			{ 
				if ( request.isDone && request.result == UnityEngine.Networking.UnityWebRequest.Result.Success )				
				{
					//csvText = request.downloadHandler.text;
					System.Text.UnicodeEncoding encoding = new System.Text.UnicodeEncoding();
					csvText = encoding.GetString(request.downloadHandler.data);
				} 		
				else 
				{
					ImportCSVWebResult = request.result.ToString() + '\n' + request.error;
				}
				m_waitingForCSVWeb = false;
			});

			while (m_waitingForCSVWeb)
			{
				yield return PowerQuest.Get.Wait();
			}

			// check for error, and return
			if ( IsString.Set(ImportCSVWebResult) )
				yield break;
		#endif
		...
		{	
				...
				writer.Flush();
				stream.Position = 0;		
				streamReader = new StreamReader(stream, System.Text.Encoding.Unicode);
			#else
			...
			using ( CSVFile.CSVReader reader = new CSVFile.CSVReader(streamReader, new CSVFile.CSVSettings() { HeaderRowIncluded = true, FieldDelimiter='	',IgnoreDimensionErrors=true,AllowNull=true  }) )
			{

				int columns = reader.Headers.Length;
				int langs = columns - CSV_NUM_HEADERS;
				m_languages = new LanguageData[langs];				
				for ( int i = 0; i < langs; ++i )
				{	
					m_languages[i] = new LanguageData() { m_code = reader.Headers[CSV_NUM_HEADERS+i], m_description = reader.Headers[CSV_NUM_HEADERS+i] };
					//Debug.Log($"Loaded header: {m_languages[i].m_description}");
				}
		...
		{
			//result = false;
			ImportCSVWebResult = "Failed to open CSV file. \nCheck it's not already open elsewhere.\n\nError: "+e.Message;
		}
		catch (System.Exception e)
		{	
			//result = false;
			ImportCSVWebResult = "Failed to import CSV file.\n\nError: "+e.Message;
		}
		finally
		{
	...
	public bool ImportFromCSV(string scriptPath, out string resultMessage)
	{
		...
		try
		{
			stream = File.OpenRead(scriptPath);
			streamReader = new StreamReader(stream, System.Text.Encoding.Default);
			CSVFile.CSVSettings settings = CSVFile.CSVSettings.TSV;
			settings.HeaderRowIncluded = false;
			using ( CSVFile.CSVReader reader = new CSVFile.CSVReader(streamReader, settings ) )
			{
				foreach ( string[] line in reader )
				{
					++lineId;
