...
public class GuiToolbarComponent : MonoBehaviour 
{
	...
	public void ToggleDialogSetting()
	{
		...
		*/
		PowerQuest.Get.Settings.LanguageId = (int)Mathf.Repeat(PowerQuest.Get.Settings.LanguageId + 1,PowerQuest.Get.Settings.GetLanguages().Length);
	...
	void SetDialogText()
	{			
		m_btnDialog.GetComponent<GuiComponent>().GetData().Description = string.Format( SystemText.Localize("Language: {0}", 82), PowerQuest.Get.Settings.LanguageData.m_description );
